/**
 * 
 */
package p1;

import java.util.List;

/**
 * Extends ChessPiece.
 * @author damonren
 * @version 1.0
 */
public class Rook extends ChessPiece {

    /**
     * Auto generated.
     */
    private static final long serialVersionUID = -6861243465389487553L;

    /**
     * Instantiates a new ChessPiece with PieceType ROOK.
     * @param player An integer.
     * @param color   The color of this ChessPiece.
     */
    public Rook(Player player, String color) {
        super(player, PieceType.ROOK, color);
    }
    
    /**
     * Rook can move horizontal or vertical unlimited number of spaces as long
     * as the way is clear of pieces.
     * 
     * @see p1.ChessPiece#move()
     */
    @Override
    List<List<Integer>> getValidMoves(final ChessPiece[][] cp) {
        validMoves.clear();
          //up
          nextX = curX - 1;
          nextY = curY;
          while ((nextX < Board.X_SIZE && nextX >= 0)
              && (nextY < Board.Y_SIZE && nextY >= 0)
              && cp[nextX][nextY] == null) {
              validMoves.add(listGenerator(nextX--, nextY));

          }
          if ((nextX < Board.X_SIZE && nextX >= 0)
              && (nextY < Board.Y_SIZE && nextY >= 0) 
              && cp[nextX][nextY] != null 
              && !isSamePlayer(cp[nextX][nextY])) {
              validMoves.add(listGenerator(nextX, nextY));
          }
          
          //down
          nextX = curX + 1;
          nextY = curY;
          while ((nextX < Board.X_SIZE && nextX >= 0)
              && (nextY < Board.Y_SIZE && nextY >= 0)
              && cp[nextX][nextY] == null) {
              validMoves.add(listGenerator(nextX++, nextY));

          }
          if ((nextX < Board.X_SIZE && nextX >= 0)
              && (nextY < Board.Y_SIZE && nextY >= 0)
              && cp[nextX][nextY] != null
              && !isSamePlayer(cp[nextX][nextY])) {
              validMoves.add(listGenerator(nextX, nextY));
          }
          
          //left
          nextX = curX;
          nextY = curY - 1;
          while ((nextX < Board.X_SIZE && nextX >= 0)
              && (nextY < Board.Y_SIZE && nextY >= 0)
              && cp[nextX][nextY] == null) {
              validMoves.add(listGenerator(nextX, nextY--));

          }
          if ((nextX < Board.X_SIZE && nextX >= 0)
              && (nextY < Board.Y_SIZE && nextY >= 0)
              && cp[nextX][nextY] != null 
              && !isSamePlayer(cp[nextX][nextY])) {
              validMoves.add(listGenerator(nextX, nextY));
          }
          
          //right
          nextX = curX;
          nextY = curY + 1;
          while ((nextX < Board.X_SIZE && nextX >= 0)
              && (nextY < Board.Y_SIZE && nextY >= 0)
              && cp[nextX][nextY] == null) {
              validMoves.add(listGenerator(nextX, nextY++));

          }
          if ((nextX < Board.X_SIZE && nextX >= 0)
              && (nextY < Board.Y_SIZE && nextY >= 0)
              && cp[nextX][nextY] != null
              && !isSamePlayer(cp[nextX][nextY])) {
              validMoves.add(listGenerator(nextX, nextY));
          }
          
          return validMoves;
    }
    
}
